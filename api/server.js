...
server.post('/api/dogs', async (req, res) => {
    try {
        ...
        if (!name || !weight) {
            res.status(422).json({
                message: `Must provide name and weight`
            })
        } else {
            const createdDog = await Dog.create({ name, weight })
            res.status(201).json({
                message: `Success creating a dog ${createdDog.name}`,
                data: createdDog,
            })
        }
    } catch(err) {
...
server.put('/api/dogs/:id', async (req, res) => {
    try {
        ...
        if (!name || !weight) {
            ...
            if (!updatedDog) {
                res.status(404).json({
                    message: `No dog exists by the given id: ${id}`
                })
            } else {
                res.status(200).json({
                    message: `Dog ${updatedDog.name}, was updated successfully`,
                    data: updatedDog
                })
            }
        }
...
server.delete('/api/dogs/:id', async (req, res) => {
    try{
        ...
        if (!deletedDog) {
            ...
            res.status(200).json({
                message: `Dog ${deletedDog.name}, was removed successfully `,
                data: deletedDog
            })
        }
    } catch(err) {
        res.status(500).json({
            message: `Error deleting a dog: ${err.message}`
        })
    }
})
